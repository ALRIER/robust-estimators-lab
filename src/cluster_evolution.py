...
def cluster_map_svg(run, generation, phase, show_inheritance=True, show_eliminated=True, show_grid=True, show_path=True):
    ...
    width, height, top, bottom = 1540, 920, 165, 790
    ...
    if phase == 1:
        active_x = xs[generations.index(generation)]
        annotations.append(f'<text x="{active_x:.0f}" y="190" text-anchor="middle" class="selectednote">SELECTED CANDIDATES</text>')
    ...
    <text x="{note_x+28}" y="{note_y+48}" class="notekicker">GA NOTATION</text>
    <text x="{note_x+28}" y="{note_y+91}" class="notetitle">{notation[0]}</text>
    <line x1="{note_x+28}" y1="{note_y+116}" x2="{note_x+337}" y2="{note_y+116}" class="noteline"/>
    <text x="{note_x+28}" y="{note_y+165}" class="formula">{notation[1]}</text>
    <text x="{note_x+28}" y="{note_y+205}" class="formula">{notation[2]}</text>
