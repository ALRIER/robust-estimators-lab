...
def cluster_map_svg(run, frame, contamination, show_inheritance=True, show_eliminated=True, show_grid=True, show_path=True):
    ...
    return f'''<html><style>body{{margin:0;font-family:Arial,sans-serif;background:#081525}}svg{{width:100%;height:480px}}.grid{{stroke:#214664;stroke-dasharray:2 6}}.guide{{stroke:#326188;stroke-dasharray:3 4}}.stage{{font-size:14px;font-weight:700;fill:#e6f2ff}}.number{{font-size:23px;font-weight:800}}.best{{fill:none;stroke:#b879ff;stroke-width:4;filter:drop-shadow(0 0 4px #7e44d7)}}.inherit{{fill:none;stroke:#79b9e6;stroke-width:2;marker-end:url(#arrow)}}</style><svg viewBox="0 0 {width} {height}"><defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3z" fill="#79b9e6"/></marker></defs>{grid}{guides}{labels}{''.join(clouds)}{inherit}{best_path}<circle cx="{target_x}" cy="{target_y}" r="66" fill="#0d4a3b" stroke="#4de080" stroke-width="2" stroke-dasharray="7 5"/><circle cx="{target_x}" cy="{target_y}" r="27" fill="#55c877" opacity=".55"/><circle cx="{target_x}" cy="{target_y}" r="8" fill="#b5ffca"/></svg></html>'''
