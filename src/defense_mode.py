...
def defense_scene_svg(scene: int) -> str:
    ...
    if scene == 0:
        ...
        flow = (("1", "Specify regime", "R = {family, n, γ, scale, mechanism}"), ("2", "Compute target", "θ = E[X]"), ("3", "Generate replicates", "x⁽¹⁾, …, x⁽ᴮ⁾"), ("4", "Apply estimator bank", "T₁(x), …, Tᴸ(x)"), ("5", "Compute squared error", "(Tⱼ(x⁽ʳ⁾) − θ)²"), ("6", "Aggregate outputs", "MSE and q95 MSE"))
        for i, (n, head, formula) in enumerate(flow):
            row, col=divmod(i,3); x=150+col*470; y=245+row*210
            body += [f'<rect x="{x}" y="{y}" width="390" height="145" rx="10" class="card"/>', _t(x+35,y+45,n,"number"),_t(x+85,y+43,head,"body"),_t(x+85,y+88,formula,"formula3")]
    else:
